      ...
      (cond ((atom val) (push val ret))
	    (t (let ((x (car val)) pkg)
		 (cond ((or (eql x :cffi-grovel-file)
			    (string-equal x "cffi-grovel:grovel-file")
			...
			(push (frob-plist-grovel val) ret))
		       ((or (eql x :cffi-wrapper-file)
			    (string-equal x "cffi-grovel:wrapper-file")
			    (and (setq pkg (cl:find-package "CFFI-GROVEL"))
				 (eql x (find-symbol "WRAPPER-FILE" pkg))))
			(push (frob-plist-wrapper val) ret))
		       ((eql x :org-cl-source-file)
			(let ((plist val))
			  (push `(:file ,@(cdr plist) :language :org-cl-source)
				ret)))
		       ((eql x :module)
...
(defun reorder-systems (in-file out-file &key (if-exists :supersede)
			(match "(mk:defsystem ")
			(compare-fn 'compare-system-names)
			dont-sort in-file-string-p
			&aux (len (length match)))
  "Reorder the systems in mk-defystem file IN-FILE which should have been
generated by asd-hack-dump-defsystem-file and write it to OUT-FILE.
This works by splitting the file into string chunks where each chunk
begins with \"(mk:defsystem <NAME><Newlin>\" line, and sorting the
chunks by the system name using COMPARE-FN.  If DONT-SORT is non-NIL,
skip the sort step.  If OUT-FILE is NIL return the chunked strings. If
IN-FILE-STRING-P is non-NIL treat IN-FILE as a string rather than a
file name."
  (assert (> (length match) 0))
  (flet ((extract-system-name (string)
	   (let* ((end (position #\Newline string))
		  (p (mismatch match string)))
	     (assert (and p (= p len)) nil "did not match a system line")
	     (loop while (eql (char string p) #\Space)
		   do (incf p))
	     (subseq string p end)))
	 (chunk (in-stream)
	   (let (first-chunk chunks line q
		 (current-chunk (make-string-output-stream)))
	     (loop (cond ((null (setq line (read-line in-stream nil nil)))
			  (let ((s (get-output-stream-string current-chunk)))
			    (when (> (length  s) 0) (push s chunks)))
			  (return))
			 ((or (null (setq q (mismatch match line)))
			      (>= q len))
			  (let ((s (get-output-stream-string current-chunk)))
			    (when (> (length s) 0)
			      (if first-chunk
				  (push s  chunks)
				  (setq first-chunk s))))
			  (write-line line current-chunk))
			 (t (write-line line current-chunk))))
	     (values first-chunk (nreverse chunks)))))
    (multiple-value-bind (first-chunk chunks)
	(if in-file-string-p
	    (with-input-from-string (in-stream in-file)
	      (chunk in-stream))
	    (with-open-file (in-stream in-file)
	      (chunk in-stream)))
      (unless dont-sort
	(setq chunks (sort chunks compare-fn :key #'extract-system-name)))
      (if (null out-file)
	  (if first-chunk (cons first-chunk chunks) chunks)
	  (with-open-file (out-stream out-file :direction :output :if-exists
				      if-exists)
	    (when first-chunk (write-string first-chunk out-stream))
	    (dolist (chunk chunks)
	      (write-string chunk out-stream)))))))
...
(defun get-recursive-deps (systems &key (seen (list :SEEN)) (depth 0)
			   (include-roots nil) &aux roots system)
...
SYSTEMS are included in the returned value."
  (dolist (system1 (if (and systems (atom systems)) (list systems) systems))
    (unless (mk-defsystem-p system1)
      (setq system (find-system system1 :load-or-nil)))
    (unless system
      (let ((name (canonicalize-system-name system1)))
	(if (find name *modules* :test #'equal)
	    (setq system name))))
    (assert system nil "System or Module ~S not found" system)
    (when (zerop depth)
      (push system roots))
    (unless (find system (cdr seen) :test #'equal)
      (setf (cdr seen) (cons system (cdr seen))))
    (when (component-p system)
      (get-recursive-deps (component-depends-on system) :seen seen :depth (1+ depth))))
  (if (zerop depth)
      (if include-roots
	  (cdr seen)
	  (remove-if (lambda (x) (member x roots)) (cdr seen)))))
